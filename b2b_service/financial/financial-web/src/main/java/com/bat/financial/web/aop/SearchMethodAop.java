...
import com.bat.financial.api.base.ErrorCode;
...
public class SearchMethodAop {
    @Pointcut("@annotation(com.bat.financial.web.annotation.SearchMethod)")
            ...
            if (arg instanceof BaseSearchQry) {
                ...
                if (contentType != null && StringUtils.isNotBlank(content)) {
                    ...
                    Method method1 = Arrays.stream(methods).filter(method -> method.getName().equals(attributeName))
                        .findFirst().orElseThrow(FinancialException::new);
                    try {
                        ...
                        e.printStackTrace();
                        throw FinancialException.buildException(ErrorCode.B_AOP_EXCEPTION);
                    }
                }
            }
            return arg;
