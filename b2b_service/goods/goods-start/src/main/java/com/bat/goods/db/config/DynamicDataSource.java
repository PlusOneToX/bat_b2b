...
import com.bat.goods.db.service.executor.TenantDBQry;
...
public class DynamicDataSource extends AbstractRoutingDataSource {
    ...
    protected Object determineCurrentLookupKey() {
        // 如果线程上下文中没有设置tenantNo,则传入默认值baseDataSource
        if (StringUtils.isNotBlank(TenantContext.getTenantNo())) {
            ...
            if (StringUtils.isBlank(dbName)) {
                ...
                if (tenantDB == null) {
                    throw GoodsException.buildException(ErrorCode.TENANT_DB_NULL);
                }
                dbName = tenantDB.getDbName();
