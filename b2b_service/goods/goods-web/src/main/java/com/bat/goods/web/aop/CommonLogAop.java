...
public class CommonLogAop extends BaseController {
    ...
    @Pointcut("@annotation(com.bat.goods.web.annotation.SysLog)")
        ...
        try {
            ...
            threadPoolTaskScheduler.submit(() -> {
                ...
                if (ex instanceof GoodsException) {
                    GoodsException goodsException = (GoodsException)ex;
                    code = goodsException.getCode();
                    msg = goodsException.getMsg();
                }
                ...
                sendService.commonLog("goods-service", sysLog.businessFunction(), businessId, platform, finalUserId,
                    userName, sysLog.value(), operateDes, operateData, new Date());
                TenantContext.removeTenantNo();
            });
        } catch (Exception e) {
