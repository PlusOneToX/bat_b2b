package com.bat.promotion.web.aop;
...
public class CommonLogAop extends BaseController {
    ...
    @Pointcut("@annotation(com.bat.promotion.web.annotation.SysLog)")
    ...
    public void afterThrowing(JoinPoint joinPoint, SysLog sysLog, Exception ex) {
        try {
            ...
            threadPoolTaskScheduler.submit(() -> {
                ...
                if (ex instanceof PromotionException) {
                    PromotionException promotionException = (PromotionException)ex;
                    code = promotionException.getCode();
                    msg = promotionException.getMsg();
                }
                ...
                sendService.commonLog("promotion-service", sysLog.businessFunction(), businessId, platform, finalUserId,
                    userName, sysLog.value(), operateDes, operateData, new Date());
                TenantContext.removeTenantNo();
            });
        } catch (Exception e) {
