 ...
 */
@Api(tags = "系统模块-区域前台接口", value = "AdminRegionController")
@Slf4j
@RestController
@RequestMapping("/system/v1/web/user/region")
public class UserRegionController extends BaseController {
    ...
    public Response<PageInfo<RegionDTO>> listRegion(@Valid RegionQry qry) {
        PageInfo pageInfo = regionService.listRegion(qry);
        return Response.of(pageInfo);
    }

    @GetMapping("/list/tree")
    @ApiOperation(value = "查询区域递归树形列表(不分页)")
    ...
    @GetMapping("/ids")
    @ApiOperation(value = "通过IDS查询多个区域")
