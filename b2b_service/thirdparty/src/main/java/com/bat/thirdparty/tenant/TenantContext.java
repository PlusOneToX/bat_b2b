package com.bat.thirdparty.tenant;
...
public class TenantContext {
    ...
    // /**
    // * 全局Map,维护不同租户对应的MongoTemplate;以【tenantNo+数据库为键,MongoTemplate对象为值】
    // */
    // public static Map<String, MongoTemplate> mongoTemplateMap = new HashMap<>();
    //
    // /**
    // * 本线程绑定的MongoTemplate,用于处理本次线程对应租户与其mongodb数据库的交互
    // */
    // public static ThreadLocal<MongoTemplate> currentMongoTemplate = new ThreadLocal<>();

    /**
     * 全局Map,维护不同租户对应的MongoDatabaseFactory;以【tenantNo+数据库为键,MongoDatabaseFactory对象为值】
     */
    public static Map<String, MongoDatabaseFactory> mongoDFactoryMap = new HashMap<>();

    /**
     * MongoDatabaseFactory,用于处理本次线程对应租户与其mongodb数据库的交互
     */
    public static ThreadLocal<MongoDatabaseFactory> currentMongoDFactory = new ThreadLocal<>();
