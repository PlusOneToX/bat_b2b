        ...
        <div class="product-all rl-clear">
          <div class="pic-box rl-fl">
            <div class="pic-move rl-margin-bottom-xxxxs rl-relative">
              ...
              <!--<img width="100%" src="../../src/assets/images/shop-detail1.png" alt="">-->
            </div>
          ...
          <div class="text-box rl-fl" v-if="goodsDetail">
            <div class="head rl-margin-bottom-mid rl-clear">
              <div class="rl-fl ding-zhi" v-if="Number(goodsType) === 2">
                <span class="img"></span>
                <span class="spc">定制</span>
              </div>
            ...
            <div class="content rl-relative">
              ...
              <div class="operate rl-clear" v-if="!this.userId == ''">
                <div v-if="this.isCollection === true" @click="joinCollect" class="collect collect1 rl-fl rl-bg-blue-xs rl-text-white rl-tc rl-text-mid cursor-pointer rl-margin-right-xxxs"><span class="img img1 rl-margin-right-xxxs"></span><span class="word">收藏</span></div>
                <div v-if="this.isCollection === false" @click="cancelCollect" class="collect collect2 rl-fl rl-bg-blue-xs rl-text-white rl-tc rl-text-mid rl-margin-right-xxxs cursor-pointer"><span class="img img2 rl-margin-right-xxxs"></span><span class="word">取消收藏</span></div>
              </div>
            </div>
          </div>
        </div>
        ...
        <el-dialog class="alls" :close-on-click-modal="diymodal" title="" :visible.sync="diyDialogVisible">
          <div class="shop-table diy-css max-height300 rl-padding-left-default rl-padding-right-default rl-clear">
            <div class="left-img rl-fl">
              <div class="diy-images"><img :src="diyPic  + '?x-oss-process=image/resize,h_104,l_104'" alt=""></div>
            </div>
            <div class="right-cons rl-fl">
              ...
              <div class="item"><span class="rl-margin-right-default">规格:</span><span>{{diyItems.specificationValueName}}</span></div>
              <div class="item"><span class="rl-margin-right-default">颜色:</span><span>{{diyItems.colorValueName}}</span></div>
              <div class="rl-clear">
...
export default {
  ...
  methods: {
    goCustrom (code) { // 获取DIY编辑器地址
      ...
      this.$api.get(this, 'user/goodsdiy/getEditUrl').then(res => {
        if (res.code === 0) {
          ...
          window.location.href = res.editorUrl + '?Token=' + res.token + '&Timestamp=' + timestamp + '&Code80=' + this.diyItemCode + '&Signature=' + Signature + '&AppID=' + AppID + '&Source=' + Source // 跳转链接
        }
      })
    },
    lookBox (code) { // 装箱规格
      this.$api.get(this, 'user/goods/getBoxInfo', {itemCode: code}).then(res => {
        if (res.code === 0) {
          ...
          } else { this.$message.warning('暂无装箱规格数据') }
        } 
      })
    },
    btnPrev () { // 定制商品分组上一步
      var box = document.getElementById('boxUl')
      var boxLi = document.getElementById('boxLi')
      if (this.setNum > 0) {
        this.offsetLeft = Number(this.offsetLeft) - Number(boxLi.offsetWidth)
    ...
    btnNext () { // 定制商品分组下一步
      var box = document.getElementById('boxUl')
      var boxLi = document.getElementById('boxLi')
      if (this.setNum < this.customizedItems.length - 8) {
        this.offsetLeft = Number(this.offsetLeft) + Number(boxLi.offsetWidth)
        box.style.left = -(this.offsetLeft) + 'px'
        this.setNum++
      }
    ...
    handleInput (target) { // 监听，字组件input自定义事件
      this.value = target
      // console.log(this.value)
    },
    showMoreLabel () { // 更多活动标签显隐
    ...
    diyShopData () {
      this.showLoading = true
      this.$api.post(this, 'user/goodsdiy/getDiyGoodsInfo', {diyId: this.diyId, goodsCode: this.goodsCode, diyNum: this.diyNum, diyPic: this.diyPic}).then(res => {
        if (res.code === 0) {
          this.showLoading = false
          ...
          if (res.diyItems) {
            ...
            Vue.set(this.diyItems, 'diyList', [])
            Vue.set(this.diyItems, 'diyId', this.diyId)
    ...
    diyBuy () {
      ...
      let product = {'diyId': this.diyId, 'diyNum': this.diyItems.count, 'diyPic': this.diyPic}
      this.diyItems.diyList.push(product)
      var limit = {'goodsId': this.diyItems.goodsId, 'itemId': this.diyItems.id, 'itemCount': this.diyItems.count, 'itemType': 3}
      this.limitPurchase.push(limit)
      ...
      this.$api.post(this, 'user/u/order/restriction', {goods: this.limitPurchase}).then(res => { // 限购规则判断
        if (res.code === 0) {
          window.localStorage.setItem('songGoodsShop', JSON.stringify(songGoodsShop)) // 赠品空数组
          window.localStorage.setItem('goodsItems', JSON.stringify(tempArrayOne))
          this.$router.push({name: 'ConsigneeInfor', query: {good_id: this.diyItems.goodsId, goodsType: this.goodsType, values: this.values}})
        }
      })
    },
    diyJoinCat () {
      let diyItemList = []
      let product = {'diyId': this.diyId, 'diyNum': this.diyNum, 'diyPic': this.diyPic}
      diyItemList.push(product)
      this.loadsing = true
      this.$api.post(this, 'user/u/shoppingCart', {goodsId: this.diyItems.goodsId, itemId: this.diyItems.id, num: this.diyNum, itemType: 3, orderType: 3, diyList: diyItemList}).then(res => {
        if (res.code === 0) {
  ...
  created () {
    ...
    this.diyId = this.$route.query.DiyID
    this.goodsCode = this.$route.query.Code80
    this.diyPic = this.$route.query.DiyPic
    this.signature = this.$route.query.Signature
    this.diyDate = this.$route.query.DiyNo
    this.source = this.$route.query.Source
    if (Number(this.source) === 0) {
      this.diyShopData()
    } else {
      window.location.href = 'http://127.0.0.1:9988/collect-web/#/diyCallback?' + window.location.href.split('?')[1]
      // window.location.href = 'http://diy.bat.com/#/diyCallback?' + window.location.href.split('?')[1]
    }
  },
  mounted () {
  }
