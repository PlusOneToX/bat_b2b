      ...
      <div class="user-main rl-clear rl-margin-zero">
        ...
        <div  class="user-right rl-fr rl-bg-white rl-margin-top-default rl-padding-bottom-double">
          <div class="order-list-wrap">
            ...
            <div class="query rl-padding-top-default" style="display:none;">
              <div class="search rl-clear">
                ...
                <!--联系人 -->
                <div class="item rl-fl rl-margin-right-xxs">  
                  <input type="text" v-model.trim="name" placeholder="联系人"/>
                </div>
                <!-- 手机号 -->
                <div class="item rl-fl rl-margin-right-xxs">
                    <input v-model.trim="mobile" onkeyup="this.value=this.value.replace(/\D/g,'')" :placeholder="$t('UserCenter.Phone')" type="text"/>
                </div>
            ...
            <div class="query-detail" style="margin-top:20px;">
              ...
              <div class="nav rl-clear">
                <ul class="rl-fl" :class="$i18n.locale === 'en' ? 'navEn' : ''">
                  <!-- 全部订单 -->
                  <li :class="orderStatus === 0 ? 'current' : ''" @click="changeState(0)" >{{ $t("UserCenter.AllOrders") }}</li> 
                  <li :class="orderStatus === 1 ? 'current' : ''" @click="changeState(1)">待审核</li>
                  <li :class="orderStatus === 2 ? 'current' : ''" @click="changeState(2)">已同意</li>
                  <li :class="orderStatus === 3 ? 'current' : ''" @click="changeState(3)">已拒绝</li>
                </ul>
                <!-- 导出订单--暂时隐藏---->
              ...
              <div class="table">
                <table>
                  ...
                  <tr v-if="orderList.length === 0 || totalCount === 0">
                    <td class="empty" colspan="7">{{ $t("P.NoData") }}</td>
                  </tr>
                  <tr v-else class="goods-list" v-for="(order, index) in orderList" :key="index">
                    <td>{{ order.orderInfo.orderNo }}</td>
                    <td>{{ order.orderDelivery.userName}}</td>
                    <td>{{ order.orderDistributorData.distributorName}}</td>
                    <td>{{ order.orderInfo.createTime | formatDate }}</td>
                    <td>
                      <template v-if="order.orderDistributorCost">
                        <i v-if="$i18n.locale === 'zh'">￥</i>
                        <i v-else>$</i>
                        {{ $i18n.locale === 'zh'?order.orderDistributorCost.payAmount:(Number(order.orderDistributorCost.payAmount)*exchange).toFixed(2) }}
                      </template>                    </td>
                    <td>{{ confirmOrderStatus(order.frontOrderStatus) }}</td>
                    <td class="rl-clear" >
                      <div class="views rl-margin-left-mid">
                        <span @click="goOrderDetail(order.orderInfo.id,order.orderDistributorData.distributorId)" class="log rl-tc cursor-pointer">{{ $t("UserCenter.ViewDetails") }}</span>
                      </div>
...
// liu--
import {orderMbnextList,orderChecktOrder, orderLists,orderTypeList,orderonemore,shoppingcartList,orderExport} from '@/apiService/api'
export default {
  name: "OrderManage",
  ...
  methods: {
    ...
    getOrderList() {
      ...
      let params={
        ...
        deliverStatus:0,  //发货状态 0.全部 1.未发货2.出库中 3.部分发货 4.已发货 (如果订单状态为已确认 才判断发货状态)
				orderStatus:this.orderStatus,  //前端订单状态 0.全部 1.待审核 2已同意 3已拒绝 4已拒绝 5已同意
    ...
    // 订单详情--y
    goOrderDetail(id,distributorId) {
      this.$router.push({ name: "distributorsOrderDetail", query: { order_id: id,distributorId:distributorId } });
    },
    confirmDialog() {
