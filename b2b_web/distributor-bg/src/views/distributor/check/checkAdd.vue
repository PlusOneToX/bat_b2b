...
<template>
  <div class="check-detail">
    ...
    <div class="box-has-border step">
      ...
      <div class="step-box">
        <el-steps :active="checkActive" finish-status="success">
          ...
          <el-step v-for="item in formData.checkFlows" :key="item.id" title="审批人" :description="item.userName">
    ...
    <div class="content" v-loading="loading" >
      ...
      <!-- 权限设置 -->
      <permissions ref="permissions" v-show="distributorTab == 2" :basicMessage="responseData" :erpFlag="erpFlag" :exaShow="exaShow" :checkMsg="checkMsg" :node="node" :checkTyps="checkTyps"></permissions>
      <!-- 联系人信息 -->
...
export default {
  ...
  computed: {
     ...
     * checkDistributorTab: 第几个子页面  0 基本信息  1 账户信息 2 权限设置 3 特价商品 4 联系人信息 5 定制设置 6 操作日志
     * isCheckStatus: 当前审批人是否自己 true:自己 false:不是自己
     * checkIsEdit: 是否可编辑是否可编辑 1.能编辑  2.不能编辑
     */
    saveShow() { // 保存是否显示
      if (this.checkMsg === '' && this.node === 1) {
        return 0 // 刷新后的页面
      } else if (this.checkMsg == 1 && this.checkDistributorTab == 0 && this.node === 1) {
        return 0 // ==== 合作中分销商 && 基本信息 ====
      } else if (this.checkMsg == 1 && this.checkDistributorTab == 1 && this.node === 1) {
        return 0 // ==== 合作中分销商 && 账户信息 ====
      } else if (this.checkMsg == 1 && this.checkDistributorTab == 2 && this.node === 1) {
        return 0 // ==== 合作中分销商 && 权限设置 ====
      } else if (this.checkMsg == 1 && this.checkDistributorTab == 3 && this.node === 1) {
        return 0 // ==== 合作中分销商 && 特价商品 ====
      }  else if (this.checkMsg == 1 && this.checkDistributorTab == 4 && this.node === 1) {
        return 0 // ==== 合作中分销商 && 联系人信息 ====
      } else if (this.checkMsg == undefined && this.node === 1) {
        return 0
      } else if (this.checkMsg == 0 && this.node === 1) {
        return 0
      }
    },
    ...
    checkDistributorTab() { // 第几个子页面  0 基本信息  1 账户信息 2 权限设置 3 特价商品 4 特价商品 5 定制商品定价 6 操作日志
  ...
  methods: {
    ...
    getDetail () {
      this.$http.distributorCheckDetail(this, {id: this.$route.query.id}).then(res => {
        if (res.success) {
          ...
          this.responseData = res.data.distributor
          if (this.formData.checkStatus === 1) {
            // 审批通过或不通过
    ...
    submit (type) {
      ...
      this.checkData.remark = this.remark
    ...
    checkRequired() { // 必填项是否完整判断
      if (this.counts === 2 && this.addSurely && this.perSurely && this.checkMsg == 1 && this.node === 2 ||
      this.counts === 3 && this.addSurely && this.accSurely && this.perSurely && this.checkMsg == 1 ||
      this.counts === 3 && this.addSurely && this.accSurely && this.perSurely && this.checkMsg == 4) { // 分销商修改
        ...
        this.agreeApprove(this.checkTypeSumbit)
        this.counts = 0
      } else if (this.counts === 3 && this.addSurely && this.accSurely && this.perSurely && this.checkMsg == 0) { // 添加分销商
        this.handleSubmitNet() // 发出判断请求
        this.counts = 0
    ...
    checkSyncs(type) { // 保存操作
      ...
      for(let i=0; i<this.$refs.roleList.tableData.length; i++) {
        if (!this.$refs.roleList.tableData[i].name || this.$refs.roleList.tableData[i].sex === undefined
          || !this.$refs.roleList.tableData[i].roleId || !this.$refs.roleList.tableData[i].password 
      ...
      this.defaultData.address.townId = this.$refs.addcomponents.formData.districtId // 区ID
      ...
      this.defaultData.business.distributorGroupId = this.$refs.permissions.formData.business.distributorGroupId // 分销商分组
      ...
      this.defaultData.extendData.erpBalanceFlag = this.$refs.permissions.formData.extendData.erpBalanceFlag // ERP余额是否同步
...
<style rel="stylesheet/scss" lang="scss" scoped>
  // @import '../cooperating/scss/cooperatingadd.scss';
  .check-detail{
    height: 100%;
    min-width: 1050px;
