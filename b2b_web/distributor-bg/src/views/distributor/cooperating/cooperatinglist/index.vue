<template>
    <div class="category-list">
      <header>
        <h4>合作中的分销商列表</h4>
        <el-button class="mini-add-btn btn-home" icon="el-icon-plus" @click="addpuls()"> 添加合作中分销商 </el-button>
      </header>

      <div class="function">
        <div class="Fheader">
          <div class="Fleft">
            <el-select size="mini"  v-model="pageInfo.autoDelivery" placeholder="是否直发" style="width: 100px;" clearable>
      ...
      <el-row v-loading="loading">
        <el-table :data="tableData" header-row-class-name="header-row" border class="tableCenter"  :height="tableHeight">
          ...
          <!-- <el-table-column label="默认价格等级" align="center" prop="gradeName" :min-width="120"></el-table-column> -->
          <el-table-column label="是否直发" :formatter="formatAutoDelivery" align="center" prop="autoDelivery" :min-width="120"></el-table-column>
          <!-- <el-table-column label="分销商区域" align="center" prop="salesAreaNames" :min-width="120"></el-table-column> -->
          ...
          <el-table-column label="操作" :min-width="170" align="center">
            <template slot-scope="scope">
              <!-- <el-button v-if="isSuperAdmin || hasPermis.actions['distributor-cooperating-get']" class="mini-search-btn" @click="handleEdit(scope.$index,scope.row)">查看</el-button>-->
...
export default {
  ...
  methods: {
    formatAutoDelivery(row, col, val){
      switch(val) {
        case 1:
          return "是";
          break;
    ...
    //  ======== 操作 ========
    addpuls() { // 添加合作中分销商操作
      this.$router.push({ name: 'distributorcooperatingadd', query: { checkMsg: 0, node:1 }})
    },
    ...
    handleEdit(index, row) { // 查看操作
      this.$router.push({ name: 'distributorcooperatingadd', query: { id: row.id, checkMsg: 1, node: 1 }})
    },
    ...
    dataFot() { // 数据列表
      this.loading = true
      // this.pageInfo.capitalStatus = 2
      // consociationData(this, this.pageInfo).then(res => {
      //   const ary = []
      ...
      // })
      // conlistData(this, this.pageInfo).then(res => { // 总列数
      //   if (res.count > 0) {
  ...
  watch: {
    "pageInfo.autoDelivery":function(){
      this.pageInfo.page = 1
      this.dataFot()
    }
  }
