<template>
  <div class="alipayAccount-list">
    <header>
      <h4>快钱支付账户</h4>
      <el-button class="mini-add-btn btn-home" icon="el-icon-plus" @click="handleAdd()"> 添加快钱账户 </el-button>
    </header>
    ...
    <el-table :data="tableData" border style="width:100%"  v-loading="loading" header-row-class-name="header-row" class="tableCenter">
      ...
      <el-table-column align="center" label="快钱人民币网关帐号" prop="merchanAcctId" show-overflow-tooltip></el-table-column>
...
export default {
  ...
  methods: {
    ...
    handleEdit(index, row) { // 查看操作
      this.$router.push({ name: 'editQuick',query: {id:row.id} })
    },

    handleAdd() { // 添加操作
      this.$router.push({ name: 'editQuick'});
    },
    ...
    handleDelete(index, row) { // 删除操作
      this.$confirm('此操作将永久删除该支付宝账户, 是否继续?', '提示', {
        ...
        this.$http.delPlatAccountQuick(this, { id: row.id }).then(res => {  
          if(res.success){
            this.$message({
              message: '删除成功',
              type: 'success',
    ...
    init() { // 列表数据
      this.loading = true;
      this.$http.platAccountQuickList(this, this.pageInfo).then(res => {    
        if(res.success){
