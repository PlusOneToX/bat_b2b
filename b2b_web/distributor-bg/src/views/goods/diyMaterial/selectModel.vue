<template>
	<div class="check-goods" >
		<div>
			...
			<el-input size="mini" style="width: 200px;float: right;" clearable @change="changeContent" placeholder="型号名称" v-model.trim="pageInfo.content" @keyup.enter.native="filter()"></el-input>
		</div>
			<el-table
			border
			class="tableCenter"
			...
			@selection-change="handleSelectionChange">
				...
				<el-table-column align="center" label="产品类型" prop="categoryName"  show-overflow-tooltip></el-table-column>
...
export default {
	...
	methods: {
		...
		getTableData() { 
			// 型号列表
			this.loading2 = true
			this.$http.modelAllList(this, this.pageInfo).then(res => {	
				this.isSelect = false
				if(res.success){
					 res.data.list.forEach(row2 => {
						row2.modelId = row2.id
						row2.bomCode = ''
						row2.materialId = ''
						row2.modelName = row2.name
						row2.materialName = ''
						row2.categoryName = row2.categoryName
						row2.length = ''
						row2.width = ''
						row2.height = ''
						row2.weight = ''
					...
					this.multipleSelect.forEach(row1 => {//重新获取数据时，判断哪些选中了
						this.tableData.forEach(row2 => {
							if(row1.modelId === row2.modelId){
								...
								row2.materialId = row1.materialId
								row2.materialName = row1.materialName
								row2.categoryName = row1.categoryName
		...
		select(selection, row){ // 单选时调用
			...
			for(let i = 0;i<this.multipleSelect.length;i++){
				if(this.multipleSelect[i].modelId === row.modelId){
		...
		setArr(arr){ // 去重
			...
			for( let i = 0; i < arr.length; i++ ) {
				let type= Object.prototype.toString.call(arr[i].modelId);//不加类型 分不清 1 '1'
