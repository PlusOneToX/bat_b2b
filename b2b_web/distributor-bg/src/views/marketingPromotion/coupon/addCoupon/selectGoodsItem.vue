<template>
	<div class="check-goods">
		<div>
			<el-select v-model="pageInfo.materialType" placeholder="材料类型" clearable size="mini" style="width: 120px;">
				<el-option
			...
			<el-select v-model="pageInfo.materialId" placeholder="选择材质" clearable size="mini" style="width: 120px;">
				<el-option
			...
			<el-select placeholder="上架状态" size="mini" style="width:120px" v-model="pageInfo.saleStatus" clearable>
			...
			<el-input size="mini" style="width: 180px;float: right;" placeholder="商品名称/商品编号/货品编号/条形码" v-model.trim="pageInfo.content" @keyup.enter.native="filter()"></el-input>
			<el-select placeholder="选择类型" size="mini" style="width:120px;float: right;" v-model="pageInfo.contentType" clearable>
				<el-option v-for="item in contentTypes" :key="item.value" :label="item.label" :value="item.value"> </el-option>
			</el-select>
		</div>
			<el-table 
			border 
...
export default {
	...
	methods: {
		...
		initData () {
			...
			// 获取材质列表
			 this.$http.materialLowestList(this, {page:1, size:1000,openFlag:1}).then(res => {		
				if(res.success){
