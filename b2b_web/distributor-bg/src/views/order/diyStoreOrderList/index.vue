<template>
  <div class="order-store-list-wrap">
    <header class="header">
      <h4>柔性定制店铺订单列表</h4>
        ...
        <div class="order-list-search">
          <div class="order-list-header">
            <div>
              ...
              <el-input
                placeholder="分销商用户名/门店名称/门店编码/收货人/订单号/公众号名称"
                @clear="clearKeyWord"
                clearable
    ...
    handleExport (type) {
      ...
      if (type === 1) {
        ...
        this.$api.post(this, "/admin/u/p/order/list", {
          page: 1,
          count: this.total,
          orderStatus: this.pageInfo.orderStatus, // 订单状态
          payStatus: this.pageInfo.payStatus, // 支付状态
          time:this.pageInfo.time,
          content: this.pageInfo.content, // 搜索关键字
          searchFrom: 'rxShopOrderList'
        }).then(res => {
          if(res.code === 0) {
