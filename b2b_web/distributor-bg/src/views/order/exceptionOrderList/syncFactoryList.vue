<template>
  <div class="order-distr-wrap">
    ...
    >
      ...
      <!-- dd :formatter="payStatusFormatter"- :formatter="orderPaymentFormatter"- :formatter="orderStatusFormatter"--->
      <el-table-column
        label="付款状态"
        align="center"
        prop="payStatus"
      ...
      >
        <template slot-scope="scope">
          <div v-if="scope.row.exceptionCauses">{{scope.row.exceptionCauses}}</div>
          <div v-else>-</div>
        </template>
      </el-table-column>
...
export default {
  ...
  data() {
    return {
      ...
      pageInfo: {
        page: 1,
        size: 10,
        errorType:2, // 同步工厂失败订单
        searchType: 7, // 同步工厂失败订单
        orderTypeId: 13,
  ...
  methods: {
    ...
    updateMainData() {
      this.loading = true;
        this.$http.syncFactoryFailList(this, this.pageInfo).then((res) => {  
      ...
      this.$http.syncFactoryFailList(this, this.pageInfo).then((res) => {  
        if (res.success) {
          ...
          import('@/utils/Export2Excel').then(excel => {
            ...
            tHeader = ['ID', '订单号', '下单时间', '分销商用户', '支付币种', '下单汇率', '支付凭证', '订单总额', '付款状态', '支付方式', '订单状态', '同步工厂失败原因']
