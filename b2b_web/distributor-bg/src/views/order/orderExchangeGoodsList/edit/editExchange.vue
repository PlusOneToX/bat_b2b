<template>
  <div class="warehouse-list-wrap">
      <div>
          <header><span>退换货单操作: 换货</span></header>
      </div>
      ...
      <div class="box-has-border" v-if="order.delivery" >
        <div class="text-center title">
            <span>收货人信息</span>
            <el-button size="mini" @click="redact()">编辑</el-button>
        </div>
        <!-- 左侧 -->
        <div class="half-width">
            <el-form ref="chosen" label-width="280px">
          ...
          <div class="edit-distribution">
            <table>
              ...
              <tr v-for="Aoge in afterServiceBean.list" :key="Aoge.id">
                ...
                <td>
                    <el-input v-model="Aoge.exchangeCount"></el-input>
                </td>
              </tr>
            </table>
...
export default {
  ...
  methods: {
    ...
    affirmAog() {
      ...
      for(let i = 0, len = this.afterServiceBean.list.length; i < len; i++){   
        var product = {
          ...
          count: this.afterServiceBean.list[i].exchangeCount
        }
    ...
    aogData() {
      ...
      this.$api.get(this, "admin/u/afterservice", { id: query.id }).then(res => {
          ...
          for(let i = 0, len = this.afterServiceBean.list.length; i < len; i++){   
            ...
            // this.payAmount = productOne - productTwo - productThree - productFour;
            // this.being = this.afterServiceBean.list[i].arrivalCount - this.afterServiceBean.list[i].exchangeCount - this.afterServiceBean.list[i].returnCount - this.afterServiceBean.list[i].againstCount;
    ...
    // 收货人编辑
    redact() {
      this.$router.push({name : 'editRedact'})
    },
    // 时间格式化
