<template>
  <div class="down-category-add">
     <header class="header">
      <h4 class="title">添加</h4>
      <el-button class="mini-add-btn btn-right" icon="el-icon-back" @click="handleBack">返回列表</el-button>
    </header>
        ...
        if (valid) {
          this.$http.addDownload(this, this.formData).then(res => {    
            if (res.success) {
              ...
              this.$router.push({name: 'downCategoryList'})
            } else {
              this.$message.error(res.msg)
            }
          })
