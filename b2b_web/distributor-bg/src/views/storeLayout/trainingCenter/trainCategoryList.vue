<template>
  <div class="train-category-list">
     <header class="header">
      <h4 class="title">培训中心分类</h4>
      <el-button class="mini-add-btn btn-right" icon="el-icon-plus" @click="handleAdd">添加分类</el-button>
    </header>
    <!----培训中心分类数据列表----->
    ...
    v-loading="loading">
      ...
      <el-table-column align="center" label="排序" :width="100" show-overflow-tooltip>
        <template slot-scope="scope">
...
export default {
  ...
  methods: {
    ...
    // 添加分类
    handleAdd() {
      this.$router.push({name : 'trainCategoryAdd'})
    },
    // 编辑
    handleEdit(row) {
      let isFlag = false
    ...
    handleSave(row) {
      if (row.sort === '') {
        ...
        this.$http.editTraining(this, params).then(res => {  
          if (res.success) {
    ...
    handleDelete(id) {
      this.$confirm('确定删除此分类吗？', '提示', {
        ...
        this.$http.delTraining(this, {id:id}).then(res => {   
					if(res.success){
						this.$message({
							message: '删除成功',
							type: 'success',
