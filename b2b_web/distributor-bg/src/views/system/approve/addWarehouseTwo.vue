<template>
	<div class="adduser">
		<el-row>
			<el-table :data="allSales" border header-row-class-name="header-row" class="adduser-table" ref='multipleSelectThree'  @selection-change="handleSelectionChange">
		...
		<el-row>
			<el-col :span="2" :offset="9">
				<el-button autofocus class="mini-search-btn" @click="handleSubmitThree"> 确定</el-button>
			</el-col>
...
import {adminAddData,adminCountData} from "@/views/system/systemData";
	export default {
		...
		methods: {
			// 选中的用户名值
			handleSubmitThree() {
				this.$emit('submitFour',this.multipleSelectThree);
			},
			// 返回
			handleBack() {
				this.multipleSelect = []
