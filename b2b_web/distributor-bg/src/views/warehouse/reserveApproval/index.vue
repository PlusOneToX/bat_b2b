  ...
  <transition name="fade" mode="out-in">
    <div class="stock-list">
      ...
      <section v-if="!coverSwitch">
        ...
        <el-table border :data="reserveds" style="width: 100%" header-row-class-name="header-row">
          ...
          <el-table-column align="center" prop="reservedStatus" label="预留状态" > </el-table-column>
          <el-table-column align="center" :formatter="formatTime" label="预留开始时间" > </el-table-column>
          <el-table-column  align="center" :formatter="formatTime" label="预留结束时间" > </el-table-column>
      ...
      <section class="pop" v-if="coverSwitch">
        <div class="half-width">
          <el-form ref="form" label-width="180px">
            ...
            <el-form-item label="审核状态"> {{checkedItem.checkStatus}} </el-form-item>
          </el-form>
        </div>
        <div class="half-width">
          <el-form ref="form" >
            <el-form-item label="申请时间"> {{timeFormat(checkedItem.createTime)}} </el-form-item>
            <el-form-item label="预留开始时间"> {{timeFormat(checkedItem.startTime)}} </el-form-item>
            <el-form-item label="预留结束时间"> {{timeFormat(checkedItem.endTime)}} </el-form-item>
            <el-form-item label="预留状态"> {{checkedItem.reservedStatus}} </el-form-item>
          </el-form>
        </div>
        <el-table :data="checkedItem.details"  style="width:100%;" border header-row-class-name="header-row">
          ...
          <el-table-column :formatter="formatCategory" label="商品分类"> </el-table-column>
          <el-table-column :formatter="formatBrand" label="品牌"> </el-table-column>
          <el-table-column prop="itemId" label="存货编码"> </el-table-column>
          <el-table-column prop="itemName" label="存货名称"> </el-table-column>
          <el-table-column prop="numInWarehouse" label="在库数量"> </el-table-column>
          <el-table-column prop="numLock" label="锁定数量"> </el-table-column>
          <el-table-column prop="numReserved" label="已预留数量"> </el-table-column>
          <el-table-column prop="numWillReserved" label="预留数量"> </el-table-column>
        ...
        <el-table :data="flows"  style="width:100%;" border header-row-class-name="header-row">
          <el-table-column type="index" fixed> </el-table-column>
          <el-table-column prop="checkUserName" label="审批人"> </el-table-column>
          <el-table-column prop="checkTime" label="审批时间"> </el-table-column>
          <el-table-column prop="checkStatus" label="审批状态"> </el-table-column>
          <el-table-column :formatter="row => {return statusFormatter(row.checkStatus)}" label="备注"> </el-table-column>
        </el-table>
        <p class="revert">
...
import {getDetailByItemIds, getReserveChecklist, getReserveChecklistCount, getCheckDetail, getStockNumByItemIds, getCategoryList, getBrandList, getAdmins, reservedCheck } from '@/views/warehouse/warehousesData'
...
export default {
  ...
  mounted(){
    ...
    this.getTotal();
    this.getReserveChecklist();
    this.getWarehousesData()
    .then(_ => {this.warehouseId = this.warehouses[0].id}) ;

    getCategoryList(this, {page: 1, count: 10000})
    .then(categoryList => {
      this.categoryList = categoryList;
    });

    getBrandList(this, {page: 1, count: 10000})
    .then(brandList => {
  ...
  methods: {
    ...
    getReserveChecklist(){
      return getReserveChecklist(this, {checkType: this.checkType})
      .then(res => {
        for(let i = 0, len = res.reserveds.length; i < len; i++){
          res.reserveds[i].details = []
        }
        this.reserveds = res.reserveds;
      })
    },
    ...
    getTotal() {
      getReserveChecklistCount(this, {checkType: this.checkType}).then(res => {
  ...
  watch: {
    ...
    // 点击查看时，请求表格需要的数据并拼接
    checkedItem(){
      // 1.首先根据预留ID发出get审批详情请求，取出审批流程数据
      getCheckDetail(this, {id: this.checkedItem.id})
      .then(res => {
        // 初始化checkUserName
        const flows = res.reserved.check.flows;
         ...
         */
        const
          itemAttrs = ['goodsName', 'categoryIds', 'brandId', 'itemName', 'boxNum', ],
          stockAttrs = ['numLock' ,'numReserved', 'numInWarehouse'],
          details = res.reserved.details,
