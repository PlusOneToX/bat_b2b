<template>
  <view class="classify">
    ...
    <!-- 筛选 -->
    <view class="qo-screen-module classifyList-navTab">
      <view
      ...
      >
        ...
        <text
          class="iconfont icon-Thepricesorted sortIcon"
          v-if="item.id == 1"
        ></text>
        <!--  @click="priceSortFun(sortWay)" -->
      </view>
      <view class="qo-screen-line">
        <text class="qo-screenBorder"></text>
        <text
          class="iconfont rowIcon"
          :class="
            rowOrLine
              ? 'icon-Classification-List'
              : 'icon-Classification-Charts'
          "
          @click="styleChoice()"
        ></text>
        <image
          src="../../static/imgs/c_line_icon.png"
            ...
            <view class="cfL-listBtm-Lf" @click="toGoodsDetails(item)">
              <template v-if="userId != ''">
                ...
                <view v-else class="cfL-listBtm-LfPrice"
                  ><text>{{ item.promotionMinPrice ? "￥" : "" }}</text
                  >{{
                    item.promotionMinPrice ? item.promotionMinPrice : "暂未定价"
                  }}
                </view>
              </template>
              <template v-else>
...
export default {
  components: { waterfallsFlow, goodsItem, tabBar },
  data() {
    return {
      title: "", //分类名
      towId: "", //二级分类id
...
.classify {
  ...
  // 横向列表
  .cfL-listModule {
    padding-top: calc(206rpx + var(--status-bar-height));
    // #ifdef H5
    padding-top: 194rpx;
    // #endif
    background: #fff;
