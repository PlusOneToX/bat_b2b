<!--
 * @Author: yaowei
 * @Date: 2019-11-13 15:05:20
 * @LastEditors: yaowei
 * @LastEditTime: 2019-05-20 15:09:25
-->
<template>
  <div class="canvas-wrapper">
    <div v-show="!showTemplate">
      <img
        ...
        alt
        class="bgimg"
        :class="{ opacity: Number(materialsType) === 2 }"
        id="bgimg"
        :width="pwidth"
        :height="pheight"
      />
      ...
      <p class="tips" v-show="showDashBorder && this.dragArr.length > 0">
        提示：请确保图片铺满{{
          Number(curPicType) !== 3 ? "红色虚线框" : "可操作区域"
        }}， <br />图片未铺满区域不打印
      </p>
    </div>
...
class DragImg {
  ...
  async paint() {
    ...
    if (this.type === 1) {
      ...
      // 添加显示层
      if (Number(materialsType) === 2) {
        // 透明图，单张图片特殊处理（绘制背景图），旋转前绘制
        bimg = document.getElementById("bgimg");
        ctx3.drawImage(bimg, 0, 0, PWIDTH * canvasRatio, PHEIGHT * canvasRatio);
...
export default {
  ...
  data() {
    return {
      ...
      clickedArea: [], // 点击热区
      hasClickedBtn: false, // 是否点击按钮（删除/移动）
  ...
  methods: {
    draw() {
      ...
      if (this.dragArr.length > 0) {
        ...
        bimg = document.getElementById("bgimg");
        if (Number(this.materialsType) !== 2) {
          ctx2.drawImage(
            bimg,
            0,
            0,
        ...
        this.dragArr.forEach((item) => {
          if (item.picType === 3 && item.type === 1 && item.picId) {
            // 模板-图片
            item.paintIpImage();
          } else {
            // 自定义/模板-文字/贴图
        ...
        if (this.initial) {
          ...
          if (this.clickedkArr.length) {
            ...
            if (
              ...
              this.hasClickedBtn = true;
              // 旋转（type: 2 文字，强制铺满血位并且type 1/picType 4 贴图/3 模板，不强制铺满血位）
              // if (
    ...
    make() {
      if (this.dragArr.length > 0) {
        ...
        let newImage = testC.toDataURL("image/png", 1.0);
    ...
    compoundImg(newImage) {
      ...
      Promise.all([
        ...
        if (Number(this.materialsType) === 2) {
          // 透明图，合并背景图
          sctx.drawImage(imgs[0], 0, 0, nwidth, nheight);
    ...
    getCutImg(imgUrl) {
      ...
      if (
        ...
        // TPU（防摔壳），合成生产图
        fCanvas.toBlob((res) => {
          let limitSize = res.size / 1024 < 50;
          let picId = this.$parent.picId;
          // 判断图片尺寸是否小于50KB
          if (!picId && limitSize) {
            this.$message.warning("作品预览失败，请重新预览~");
            this.$parent.message = "";
    ...
    importFile(image, pdfimg) {
      ...
      let distributor = localStorage.getItem("distributor");
      if (
        baseUrl.indexOf("diy.bat.com") > 0
      ) {
        this.uploadFile(imgFile, pdfFile, distributor);
      } else {
        this.uploadFile(imgFile, pdfFile, distributor + "_test");
      }
    },
    getImgDPI(item) {
      // 得到手机尺寸（英寸）
    ...
    limitScale(touchEvent, initial, startTouch, lastImg) {
      ...
      let isAllPlace = parseInt(sessionStorage.getItem("isAllPlace"));
      // 获取最小缩放宽高，非模板
      // let minW = (isAllPlace && lastImg.picType !== 3) ? lastImg.minW : 20;
      // let minH = (isAllPlace && lastImg.picType !== 3) ? lastImg.minH : 20 * (lastImg.minW / lastImg.minH);
    ...
    // 点击热区
    // handlePic(e) {
    //   if (this.curPicType === 3 && !this.hasClickedBtn) {
    ...
    spriteArr(arr) {
      ...
      if (arr.length > 0) {
        ...
        if (parseInt(newImg.picType) === 2) {
          ...
          if (this.dragArr.length > 0) {
            this.dragArr.forEach((drag, index) => {
              // type 1--图片
...
.clicked-area {
  ...
  .icon-add_picture {
    width: 26px;
    height: 26px;
    background: url('../../common/images/temp/add_picture.png') no-repeat;
  ...
  .tips {
    ...
    p {
      ...
      .icon-tihuan {
        background: url('../../common/images/temp/icon_tihuan.png') no-repeat;
        background-size: 100% 100%;
      }

      .icon-bianji {
        background: url('../../common/images/temp/icon_replace.png') no-repeat;
        background-size: 100% 100%;
      }
    }
  }
