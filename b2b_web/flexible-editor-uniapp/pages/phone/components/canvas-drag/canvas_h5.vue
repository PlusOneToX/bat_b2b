<template>
    <view>
        ...
        <!-- 右边框 -->
        <cover-view :class="Isopacity?'iscanvas_copy':''" :style="{width:((750-widthVar)/2)+2+'rpx',height:modelHeight+'rpx',marginTop:'100rpx'}" class="cover_view2"></cover-view>
        <!-- 下边框 -->
        ...
        <view class="up-box" @touchstart="start" @touchmove="move" @touchend="end" :style="{
            backgroundImage: 'url(' + backgroundImage + ')',  backgroundSize: widthVar+'rpx ' + modelHeight + 'rpx', width: '750rpx', height: windowHeight+'rpx'}"></view>
        <view class="up-box1" :style="{backgroundImage: 'url(' + floorImage + ')',  backgroundSize: widthVar+'rpx ' + modelHeight + 'rpx',  width: '750rpx', height: windowHeight+'rpx' }"></view>
...
export default {
    ...
    methods: {
        ...
        changeBgImage(newBgImg, e) {
            this.drawArr.forEach(item => {
                if (item.fileUrl == e) {
                    item.fileUrl = newBgImg;
                }
            })
            // this.bgColor = '';
