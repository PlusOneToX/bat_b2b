<template>
    <view>
        <canvas canvas-id="canvas-drag" :drawData="drawArr" disable-scroll="true" :style="{width: '750rpx', height: windowHeight-400+'rpx',zIndex: 1}"></canvas>
    </view>
</template>
...
dragGraph.prototype = {
    ...
    paint() {
        ...
        if (this.type === 'text') {
            ...
            var bs = uni.getStorageSync("bs");
            if (this.fileUrl == bs) {
                var r = 30;//弧度大小
                this.ctx.save();
                if (this.w < 2 * r) r = this.w / 2;
                if (this.h < 2 * r) r = this.h / 2;
                this.ctx.beginPath();
                this.ctx.moveTo(this.x + r, this.y);
                this.ctx.arcTo(this.x + this.w, this.y, this.x + this.w, this.y + this.h, r);
                this.ctx.arcTo(this.x + this.w, this.y + this.h, this.x, this.y + this.h, r);
                this.ctx.arcTo(this.x, this.y + this.h, this.x, this.y, r);
                this.ctx.arcTo(this.x, this.y, this.x + this.w, this.y, r);
                this.ctx.closePath();
                this.ctx.clip();
            }
            ...
            return new Promise((resolve, reject) => {
                ...
                this.draw().then(() => {
                    uni.canvasToTempFilePath({
                        ...
                        destWidth: (this.widthVar / ratio) * devicePixelRatio,//this.width * 7, // 生成的图片多宽
                        destHeight: (this.modelHeight / ratio) * devicePixelRatio,//this.height * 7, // 生成的图片多高
