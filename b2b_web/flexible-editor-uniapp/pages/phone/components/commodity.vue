    ...
    <uni-popup ref="popup" type="bottom" :safeArea="false" @maskClick="close">
        <view class="sele_img_box" v-if="IsPhone">
            ...
            <view class="commodity_message_box">
                <view class="price_box">￥{{Price}}</view>
                    ...
                    <view>
                        ...
                        <scroll-view class="scroll_view_y1" :show-scrollbar="false" :scroll-y="true">
                            <view class="texture_box" style="margin-top: 18rpx;">
                                ...
                                <view @click="selectmaterials(item)" v-for="(item,index) in materialsList" :key="index">
                                    <uni-badge v-if="item.underStockFlag==1" class="uni-badge-left-margin" text="缺货" absolute="rightTop" size="small" type="info">
                                        <view class="texture_meu" :class="item.id==materialsId?'select':(item.modelFlag==1||item.underStockFlag==1?'':'noselect')">
            ...
            <view class="select_commodity_box" @click="confirm">确定</view>
        </view>
    ...
    methods: {
        ...
        getsubordinateList() {
            this.$api.get(this, api.getNewmaterialList, {
                ...
                var index = 0;
                this.materialsList.forEach(item => {
                    if (item.id == this.materialsId && item.modelFlag == 1 && index == 0) {
                        index = 1;
                        this.materialsId = item.id;
                        this.manufactors = item.manufactor;
                        this.materialsName = item.name;
                ...
                // underStockFlag//0不缺货 1缺货
                // if (res.data[0].modelFlag == 1) {
                //     this.materialsId = this.materialsList[0].id;
                //     this.manufactors = this.materialsList[0].manufactor;
                //     this.materialsName = this.materialsList[0].name;
                //     this.itemCode = this.materialsList[0].itemCode;
                //     this.itemId = this.materialsList[0].itemId;
                //     this.Isspecific = true;
                //     this.getPrice();
                // } else {
                //     this.Isspecific = false;
