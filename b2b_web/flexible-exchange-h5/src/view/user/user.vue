<template>
  <div class="user">
    <THeader class="header-wrap" :hasBg="true" :title="'用户详情'"></THeader>
    <div class="container">
      ...
      <div class="login-out" @click="logout" v-show="platform !== '7'">
        退出登录
      </div>
      <!---修改昵称---->
      ...
      <!---手机验证,更换手机号---->
      <div class="setphone-wrapper" :class="fadephone ? 'show' : ''">
        <THeader
          :back="true"
          :title="title"
          :hasBg="true"
...
import { inputBlur } from "common/js/common";
...
export default {
  ...
  data() {
    return {
      ...
      userInfo: {
        ...
        sex: 0,
        birthday: "",
        headPortrait: require("../../common/images/user.png"),
        phone: "",
        password: "",
        id: 0,
        no: 0,
  ...
  mounted() {
    ...
    this.userInfo.no = localStorage.getItem("userNo") || "";
  ...
  methods: {
    ...
    async afterRead(data) {
      ...
      //   ]).then(img => {
      //     // 压缩图片
      //   let pic = this.handleImgScale(img[0])
      //     console.log('im--', file)
      //   })
      // })
      // let reader = new FileReader()
      // let red = reader.readAsDataURL(file)
      // // let pic = handleImgRotate(red.result)
      // console.log('file--', file, red)
      // 压缩图片
      // let img = this.handleImgScale(file.name)
    ...
    getUserInfo() {
      this.$store
        .dispatch("GetInfo", { userId: this.userInfo.id })
        .then((res) => {
          if (res.status === 200) {
            this.userInfo = res.data;
            // if (this.userInfo.headPortrait === '' || this.userInfo.headPortrait === null || this.userInfo.headPortrait === undefined) {
    ...
    updateUserInfo(type) {
      this.$api
        .put(this, api.updateInfo, this.userInfo)
        .then((res) => {
          if (res.status === 200) {
          ...
          this.$store
            ...
            .then(() => {
              ...
              if (platform && platform === "7") {
                ...
                // this.$router.push({name: 'login', query: {key: '-1'}})
              }
            })
            .catch((error) => {
              console.log(error);
    ...
    getCode(type) {
      ...
      if (flag) {
        this.$api
          .post(this, api.captchaSend, data)
          .then((res) => {
            if (res.status === 200) {
