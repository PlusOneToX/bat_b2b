<template>
  ...
  <!-- <div id="update" v-else>
    <div class="system-wrap">
      <img src="./assets/images/system-update.gif" alt="系统升级中..." />
      <p>我们正在升级系统服务</p>
      <p>请稍后再来</p>
    </div>
  </div> -->
...
export default {
  ...
  mounted() {
    ...
    let platform = this.getUrlParams("platform");
    if (Number(platform) === 7) {
      if (this.getChromeVersion()) {
        var version = this.getChromeVersion();
        if (version <= 60) {
          sessionStorage.setItem("isLowerVersion", 1);
          this.$dialog
            .confirm({
              title: "温馨提示",
              message: "系统浏览器内核版本过低，暂不支持访问。",
  ...
  methods: {
    ...
    initTenantInfo() {
      ...
      let params = {
        ...
        qryUrlType: 7, // 传参主机类型(传参主机host不为空时填，默认3)：1-分销后台PC端 2-分销前台PC端 3-分销前台H5端 4-店铺二维码 5-分销商申请二维码 6-后端接口 7 柔性H5端 9兑换商城H5端
        host: host, // 主机
      };
