  ...
  <div class="template-wrap">
    ...
    <div class="hsb-wrap" v-show="showHSB">
      <div class="slider-wrap">
        <!-- 对比度 -->
        <van-slider v-show="curHSB === 0" v-model="curContast" :min="-180" :max="180" :step="1" active-color="#0076A5"
          @input="getContrast()">
        </van-slider>
        <!-- 亮度 -->
        <van-slider v-show="curHSB === 1" v-model="curBrighten" :min="-1" :max="1" :step="0.05" active-color="#0076A5"
          @input="getBrighten()">
        </van-slider>
        <!-- 饱和度 -->
        <van-slider v-show="curHSB === 2" v-model="curSaturation" :min="-10" :max="10" :step="0.1"
          active-color="#0076A5" @input="getSaturation()">
        </van-slider>
      </div>
      <ul class="handle-list">
        <li @click="curHSB = 0">
          <span class="sprite-icon"
            :class="(curHSB === 0) ? 'template_icon_contrast_pre': 'template_icon_contrast'"></span>
          <p>对比度</p>
        </li>
        <li @click="curHSB = 1">
          <span class="sprite-icon "
            :class="(curHSB === 1) ? 'template_icon_brightness_pre': 'template_icon_brightness'"></span>
          <p>亮度</p>
        </li>
        <li @click="curHSB = 2">
          <span class="sprite-icon "
            :class="(curHSB === 2) ? 'template_icon_saturation_pre': 'template_icon_saturation'"></span>
          <p>饱和度</p>
        </li>
      </ul>
      <div class="bottom-wrap">
