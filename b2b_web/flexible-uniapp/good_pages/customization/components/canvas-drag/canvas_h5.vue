<template>
    <view>
        <canvas canvas-id="canvas-drag" class="canvas_drag" :drawData="drawArr" disable-scroll="true" :style="{width: '750rpx', height: original+'rpx',zIndex: 1}"></canvas>
        <view @touchstart="start" @touchmove="move" @touchend="end" :class="Isopacity?'iscanvas_copy':''" class="image_box" :style="{height:modelHeight+'rpx',borderTop: marginTopVar+'rpx solid #efeff4',borderLeft: leftFrameVar+'rpx solid #efeff4',borderRight: (leftFrameVar)+'rpx solid #efeff4',borderBottom: (marginTopVar+4)+'rpx solid #efeff4'}">
            <image class="hollow_out" :src="hollowOut" :style="{width:(widthVar+4) + 'rpx',height:modelHeight+'rpx'}"></image>
        </view>
        <view class="up-box" @touchstart="start" @touchmove="move" @touchend="end" :style="{
            backgroundImage: 'url(' + backgroundImage + ')',  backgroundSize: widthVar+'rpx ' + modelHeight + 'rpx', width: '750rpx', height: original+'rpx',  backgroundPosition: '50% '+marginTopVar+'rpx'}"></view>
        <view class="up-box1" :style="{backgroundImage: 'url(' + floorImage + ')',  backgroundSize: widthVar+'rpx ' + modelHeight + 'rpx',  width: '750rpx', height: original+'rpx' , backgroundPosition: '50% '+marginTopVar+'rpx'}"></view>
        <Coverage ref="set_soverage" @onCoverage="initByArr"></Coverage>
    </view>
</template>
...
const dragGraph = function ({
    ...
    imaType,
    id,//图片id
    source,//图片来源：1:可画图片；2:网络图片：3：本地上传图片
}, canvas, factor) {
    if (type === 'text') {
...
dragGraph.prototype = {
    ...
    paint() {
        ...
        if (this.type === 'text') {
            ...
            //下划线
            if (this.wordStyle.textDecoration == 'underline') {
                this.ctx.strokeStyle = this.color;
                this.ctx.lineWidth = 2;
                this.ctx.moveTo(this.x, this.centerY + (textHeight / 2.4));
                this.ctx.lineTo(this.x + textWidth, this.centerY + (textHeight / 2.4));
                this.ctx.stroke();
            }
        ...
        proportion: {
            handler(newName, oldName) {
                if (oldName != undefined) {
                    ...
                    this.leftFrameVar = ((750 - this.widthVar) / 2) + 1;
                    this.marginTopVar = (this.original - this.modelHeight) / 2;
                }
            },
            immediate: true
        }
        ...
        draw() {
            ...
            if (this.drawArr.length > 1 && this.drawArr[this.drawArr.length - 1].imaType == 'map') {
                ...
                this.drawArr.forEach(item => {
                    if (item.imaType == 'map') {
                        if (!uni.getStorageSync("Isreplace")) {
                            isreplace = item;
                        } else {
                            mapList.push(item);
                        }
            ...
            return new Promise((resolve, reject) => {
                ...
                this.draw().then(() => {
                    ...
                    uni.canvasToTempFilePath({
                        y: (this.marginTopVar + (this.topFrame * 6)) / ratio,
...
.hollow_out {
    margin: 0 auto;
    z-index: 8;
    box-shadow: 0 0 0 2rpx #efeff4;
    // box-shadow: 0 0 0 10rpx #efeff4;
}
.iscanvas_copy {
    opacity: 0.9;
}
