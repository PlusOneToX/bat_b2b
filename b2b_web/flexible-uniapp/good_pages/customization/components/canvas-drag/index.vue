<template>
    <view>
        <!-- &&hollowOut!='' -->
        <view v-show="Iscanvas&&NumberPictures==false">
            <canvas canvas-id="canvas-drag" class="canvas_drag" :drawData="drawArr" disable-scroll="true" :style="{width: '750rpx', height: windowHeight+'rpx',zIndex: 1}"></canvas>
            <!-- 上边框 -->
            <cover-view :class="Isopacity?'iscanvas_copy':''" :style="{height:'100rpx'}" class="cover_view"></cover-view>
            <!-- 左边框 -->
            <cover-view :class="Isopacity?'iscanvas_copy':''" :style="{width: (750-widthVar)/2+'rpx',height:modelHeight+'rpx',marginTop:'100rpx'}" class="cover_view1"></cover-view>
            <!-- 右边框 -->
            <cover-view :class="Isopacity?'iscanvas_copy':''" :style="{width:((750-widthVar)/2)+4+'rpx',height:modelHeight+'rpx',marginTop:'100rpx'}" class="cover_view2"></cover-view>
            <!-- 下边框 -->
            <cover-view :class="Isopacity?'iscanvas_copy':''" :style="{width:'750rpx', marginTop:(100+modelHeight)+'rpx',height:'104rpx'}" class="cover_view1"></cover-view>

            <canvas canvas-id="canvas-drag1" @touchstart="start" @touchmove="move" @touchend="end" disable-scroll="true" style="position: absolute;" :style="{width: '750rpx', height: windowHeight+'rpx',zIndex: 11}"></canvas>
            <cover-image class="cover_image" :src="backgroundImage" :style="{width:widthVar + 'rpx',height:modelHeight+'rpx',left:leftFrameVar+'rpx',marginTop: '100rpx'}"></cover-image>
            <cover-image :class="Isopacity?'iscanvas_copy':''" class="hollow_out" :src="hollowOut" :style="{width:widthVar + 'rpx',height:modelHeight+'rpx',left:leftFrameVar+'rpx',marginTop: '100rpx'}"></cover-image>
            <cover-image class="cover_image_floorImage" :src="floorImage" :style="{width:widthVar + 'rpx',height:modelHeight+'rpx',left:leftFrameVar+'rpx',marginTop: '100rpx'}"></cover-image>
        </view>
        ...
        <Coverage ref="set_soverage" @onCoverage="initByArr" @Iscanvas="canvasIs"></Coverage>
    </view>
</template>
...
dragGraph.prototype = {
    ...
    paint() {
        ...
        let textWidth = 0;
        let textHeight = 0;
        // this.ctx.fillRect(0, 0, 350, 750);
        // this.ctx.clearRect(20, 70, 200, 350);
        if (this.type === 'text') {
            var fontStyle = '';
            ...
            if (this.wordStyle.fontWeight == 600) {
                this.ctx.font = fontStyle + ' bold 20px ' + this.wordStyle.fontFile;
            } else {
                this.ctx.font = fontStyle + ' 400 20px ' + this.wordStyle.fontFile;
            }
...
export default {
    ...
    methods: {
        ...
        draw() {
            ...
            if (this.drawArr.length > 1 && this.drawArr[this.drawArr.length - 1].imaType == 'map') {
                var mapList = [];
                var drawList = [];
                ...
                // if (this.NumberPictures == false && mapList.length >= 2) {
                //     this.$refs.message.open();
                //     this.NumberPictures = true;
                //     return
                // }
                // this.NumberPictures = false;
                var data = [...mapList, ...drawList];
                this.drawArr = data;
                this.drawArr.forEach(item => {
                    item.paint(item);
                });
        ...
        start(e) {
            ...
            // const x = e.touches[0].clientX == 0 ? e.touches[0].pageX : e.touches[0].clientX;
            // const y = e.touches[0].clientY == 0 ? e.touches[0].pageY : e.touches[0].clientY;
            ...
            this.drawArr && this.drawArr.forEach((item, index) => {
                ...
                const action = item.isInGraph(x, y, length);
                if (action) {
                    this.$emit("dpi", item);
                    item.action = action;
            ...
            return new Promise((resolve, reject) => {
                ...
                this.draw().then(() => {
                    ...
                    uni.canvasToTempFilePath({
                        y: (100 + (this.topFrame * 6)) / ratio,
                        x: (x + (this.leftFrame * 6)) / ratio,
                        width: (this.widthVar - ((this.leftFrame * 2) * 6)) / ratio, // 选中canvas多宽
                        height: (this.modelHeight - ((this.topFrame * 2) * 6)) / ratio, // 选中canvas多高
                        destWidth: ((this.widthVar - ((this.leftFrame * 2) * 6)) / ratio) * devicePixelRatio,//this.width * 7, // 生成的图片多宽
                        destHeight: ((this.modelHeight - ((this.topFrame * 2) * 6)) / ratio) * devicePixelRatio,//this.height * 7, // 生成的图片多高
...
.canvas_copy:before {
    top: var(--margin-top-var);
    margin-left: var(--left-frame-var);
    width: var(--width-var);
