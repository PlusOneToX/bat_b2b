    ...
    <view class="coupon">
        <view class="container-wrapper">
            ...
            <view class="coupon-list">
                <view class="title">
                    <view class="title-name">618新机用户专享福利</view>
                </view>
...
export default {
    ...
    methods: {
        ...
        getInvalidCoupon(xSaUid) {
            this.$api.get(this, api.getValidCoupon, {
                accountId: xSaUid,
            })
                .then((res) => {
                    if (res.success) {
        ...
        _initDate(couponTypes, statuss) {
            if (this.userId) {
                this.$api.get(this, api.getSpecialCoupon, {
                ...
                uni.hideLoading()
                uni.navigateTo({
                    url: "../login/login?platform=" + this.platform + "&distributorId=" + this.distributorId + "&sa=3&ids=" + this.ids + "&samFlag=samA52Flag&hasQualified=" + this.hasQualified
                });
            }
        },
        onSamLogin(saInfo) {
        ...
        getCoupon(ids) {
            ...
            this.$api.post(this, api.receiveCoupon, { ids: this.ids }).then((res) => {
                if (res.success) {
                    // 领取成功 - 跳转首页
                    // uni.setStorageSync("sa", "1");
                    uni.showToast({
                    ...
                    let timer1 = setInterval(() => {
                        uni.navigateTo({
                            url: '/good_pages/discount_coupon/discount?discount=1'
                        });
                        clearTimeout(timer1)
                    }, 1500);
                } else {
  ...
  .coupon {
      ...
      .container-wrapper {
          ...
          .header-img {
              ...
              width: 100%;
              height: 230px;
              background-image: url('https://bat-com.oss-cn-shenzhen.aliyuncs.com/diy/h51685070198354.blob:http://localhost:8080/a0c0cd18-2585-41a0-9fbf-b3228abd340c');
